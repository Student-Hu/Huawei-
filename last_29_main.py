    ...
    for k in range(road_channel):
        for car_in_road in caing_start2end['id'].values.tolist():
            if current[(current['from'] == start_id) & (current['to'] == end_id)].shape[0] <= int(
                ...
                enough = False  #
                roading = current[
                    (current['from'] == start_id) & (current['to'] == end_id) & (current['run_channel'] == k + 1)]
                if roading.empty:
                    S = road_length
                else:
                    roading = roading.sort_values(["weizhi"], ascending=True)  # 按照位置深序
                    S = roading["weizhi"].iloc[
                            0] - 1  # 最后一辆车的位置                #                            print(S)
                ...
                if S > 0:  # 能放
                    ...
                    caing_start2end = caing_start2end[
                        ~caing_start2end['id'].str.contains(car_in_road)]  # 带出发的车删除这个车
                    continue
                else:  # 这条道不能放
                    break
            else:
...
for j in range(car.shape[0]):
    ...
    while True:
        T = T + 1
        #################出库+行驶模型#########################
        caring = car[(car['planTime'] == str(T))]
        caring = caring.sort_values(["id"], ascending=True)
        if len(jieguo) != 0:
            break
            #################单一道路模型#########################
        Z_luxian.update({car.iloc[j]['id']: {'path': path, 'path_road': path_road}})
        current = pd.DataFrame(
            columns=['carid', 'run_road', 'from', 'to', 'run_channel', 'weizhi', 'state', 'next_road'])
        for index, row in road.iterrows():  # 以起始点判断那一条路
            start_id = row['from']
            end_id = row['to']
            caing_start2end = caring[
                (caring['from'] == start_id) & (caring['path1to'] == end_id)]  ##这一时刻要在这个form-to出发的所有车
        ...
        while current[current['state'] == 1].shape[0] != 0:  # 当前时刻所有车辆不是终止状态的话
                    ...
                    while 1:
                        ...
                        # 对面道路第一条道路最后一辆的信息
                            ...
                            next_going_road = current[
                                (current['from'] == cross['id'][cross_index]) & (
                                        current['run_road'] == next_going_road_id)]
                            for k in range(
                                    int(road[road['id'] == next_going_road_id]['channel'].iloc[
                                            0])):  # 判断当前将要驶入的道路总共有多少车道
                                ...
                                if road_of_channel.empty:
                                    next_road_speed = \
                                        road[(road['id'] == next_going_road_id)]['speed'].values.tolist()[
                                            0]  # 下一条道路限速
                                    car_speed = car[car['id'] == In_road['carid'][index]]['speed'].values.tolist()[
                                        0]
                                    ...
                                    V2 = int(min(int(next_road_speed), int(car_speed)))  # 在next road最大行驶速度
                                    road_length = int(
                                        road[road['id'] == In_road['run_road'][index]]['length'].values.tolist()[
                                            0])  # 当前道路的长度
                                    ...
                                    if S > 0:  # 当前车道没满
                                        if state == 0:  # 当车为终止状态时
                                            ...
                                            if S2 <= 0:  # 不能通过路口
                                                ...
                                                current['state'][index] = 0  # 终止
                                                location_path = Z_luxian[current['carid'][index]][
                                                    'path_road'].index(
                                                    current['run_road'][index])
                                                ##进入的下一条道路包括（from，to, 路的id,下一条路id,方向，第几条到）
                                                current['run_road'][index] = \
                                                    Z_luxian[current['carid'][index]]['path_road'][
                                                        location_path + 1]
                                                if len(Z_luxian[current['carid'][index]][
                                                           'path_road']) > location_path + 2:
                                                    current['next_road'][index] = \
                                                        Z_luxian[current['carid'][index]]['path_road'][
                                                            location_path + 2]
                                                current['from'][index] = Z_luxian[current['carid'][index]]['path'][
            ...
            if len(last_state_id) != len(present_state_id):
                sisuo = True
                break
            else:
